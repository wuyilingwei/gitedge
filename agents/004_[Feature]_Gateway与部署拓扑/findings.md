...
- `[可信身份]` -> 客户端可能伪造 `X-GitEdge-User-*` -> Gateway 转 Forge 前删除这些头，只接受 Auth `/session` JSON 返回的身份 -> Forge 不依赖客户端 Cookie 或未经验证的身份头。
- `[Git Worker 边界]` -> 上游协议实现可按路由注册 -> `workers/git/src/index.ts` 只注册 Git Smart HTTP 与 queue/DO 导出 -> 保留现有 DO/R2/D1/Queue 与 Git 协议实现，不暴露旧 SSR/OIDC/admin 路由。
