...
describe("GitEdge API client", () => {
  ...
  it("translates UI fields into the Forge repository contract", async () => {
    ...
    await api.createRepository({ owner: "rosmontis", name: "edge", description: "At the edge", visibility: "public" });

    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/forge/repositories",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ slug: "edge", owner: "rosmontis", description: "At the edge", visibility: "public" }),
      })
    );
  });
  ...
  it("exposes organization endpoints and preserves the owner namespace", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, "/api/forge/organizations/acme/members", expect.anything());
  });
